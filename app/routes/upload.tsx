...
const Upload=()=>{
    const {auth,isLoading,fs,ai,kv} =usePuterStore(); // here fs=file storage, kv= key value storage
    ...
    const handleAnalyze= async ({companyName,jobTitle,jobDescription, file}:{companyName:string, jobTitle:string, jobDescription:string, file:File})=>{
        ...
        const uploadedFile= await fs.upload([file]); //here fs directly coming from puter
        //if error uploading file to puter
    ...
    const handleSubmit=(e:FormEvent<HTMLFormElement>)=> {
        e.preventDefault();
        const form= e.currentTarget.closest('form');// this gives us form data without relying on state
        if(!form) return;
